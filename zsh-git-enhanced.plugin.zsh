...
alias gbda='git branch --no-color --merged | command grep -vE "^([+*]|\s*($(git_main_branch)|$(git_develop_branch))\s*$)" | command xargs -n 1 git branch -d 2>/dev/null'
alias gbD='git branch -D'
...
alias gsd='git rebase -i HEAD~$(git rev-list --count origin/$(git_main_branch)..$(git rev-parse --abbrev-ref HEAD))'
alias gsh='git show'
...
alias grlm='git rev-list --count main..$(git_current_branch)'
alias grlmo='git rev-list --count origin/main..$(git_current_branch)'
